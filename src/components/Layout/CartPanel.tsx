...
const CartPanel: React.FC<CartProps> = ({ open, onClose }) => {
  ...
  if (!context) <div>Данных контекста пока нет, попробуйте позже</div>;
  const {
    address = null,
    setAddress = () => {},
    setCartId = () => {},
  ...
  const CartItemsList = (
    ...
    >
      <Box
        sx={{ flex: 1, overflowY: "auto", width: 350, bgcolor: "whitesmoke" }}
        role="presentation"
      >
        <List sx={{ width: "100%", maxWidth: 350 }}>
          {cart?.items.map((orderItem) => {
            ...
            const ingredientList = getAddedIngredients(
              orderItem.addedIngredientIds
            );
            return (
      ...
      >
        <Typography marginLeft={"7px"} marginTop={"7px"} variant="h6">
          Итого: {cart?.totalPrice.toFixed(2)} ₽
        </Typography>
        <Button
          variant="contained"
          style={{ marginTop: 16, marginLeft: 7, marginBottom: 7 }}
          onClick={handleClickOpen}
          disabled={!cart?.totalPrice || cart?.totalPrice <= 0}
        >
          Оформить заказ
        </Button>
      </Box>
  ...
  return (
    <div>
      ...
      />
      <Dialog open={openConfirm} keepMounted onClose={handleClose}>
        ...
        <DialogContent>
          <DialogContentText component="div">
            ...
            <Autocomplete
              freeSolo={false}
              options={suggestions}
              getOptionLabel={(option: Suggestion) => option.value}
              ...
              renderInput={(params) => (
                <TextField
                  ...
                  error={!fiasId}
                  helperText={!fiasId ? "Выберите адрес из списка" : ""}
        ...
        <DialogActions>
          ...
          <Button onClick={confirmOperation} disabled={!address}>
            Подтвердить
          </Button>
        </DialogActions>
      </Dialog>
